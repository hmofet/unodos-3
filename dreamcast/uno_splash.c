/* ===========================================================================
 * UnoDOS/Dreamcast milestone-0 splash - the "hello-PVR" screen.
 *
 ...
 * controller position. Shared verbatim by the host shim (renders once -> PNG)
 * and the DC target (renders each vblank at the live cursor -> PVR upload). M1
...
void uno_render_splash(int cx, int cy)
{
    ...
    fb_text(FB_W - 8 - fb_text_w("Dreamcast"), 6, "Dreamcast", UNO_BLACK, -1);
    ...
    center_text(py + 90,  "Sega Dreamcast port", UNO_BLACK, -1);
    center_text(py + 108, "Milestone 0  -  hello-PVR", UNO_BLACK, -1);
    center_text(py + 134, "640x480 software framebuffer  ->  PVR", UNO_BLUE, -1);
