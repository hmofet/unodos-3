...
#
#   build/uno_realcore_dc   - the app-free kernel, -rdynamic so modules resolve it
...
# tools/unodos_orig_dc.c is the pristine pre-refactor core kept for reproducible
# regeneration; if absent, seed it from the current unodos.c.
[ -f tools/unodos_orig_dc.c ] || cp unodos.c tools/unodos_orig_dc.c
"$PY" tools/refactor_core.py tools/unodos_orig_dc.c unodos.c

echo "[3/6] build the REAL core (no app code, -rdynamic)"
$CC -O2 -DUNO_COLOR=1 -DUNO_HOST -I. -rdynamic \
    -o build/uno_realcore_dc \
...
UNO_OUT=shots/real_desktop.ppm ./build/uno_realcore_dc
...
for pair in $NAMES; do
    id=${pair%%:*}; name=${pair##*:}
    UNO_APP=$id UNO_OUT=shots/real_$name.ppm ./build/uno_realcore_dc
