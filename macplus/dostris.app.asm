; ============================================================================
; UnoDOS/MacPlus Dostris - DISK-LOADED app (DOSTRIS.APP, proc 5, slot 1).
; Falling-block game; logic ported verbatim from the in-kernel games.i. State
; (dt_*) lives in the kernel vars (shared with the kernel's tick gate); the
; game music sequencer (gm_*) is kernel-side (audio). 1-bit dither colours.
; ABI: JMP draw/key/tick/click. mkapp.py converts kernel bsr/bra -> jsr/jmp and
; de-PC's kernel-symbol refs.
; ============================================================================
...
DT_CELL     equ 8
        ...
        org     APP_LOAD+APPSLOT_DOSTRIS*APP_SLOT_SZ
        dc.l     dostris_draw        ; +0
        dc.l     dostris_key         ; +4
        dc.l     dostris_tick        ; +8
        dc.l     dostris_clickv      ; +12
dostris_clickv:
        rts
...
.hdone: bsr     dt_lock
.redraw:
        moveq   #0,d0               ; consumed; the kernel redraws topmost
        rts
