...
; Contract-owned (CONTRACT-ARCH Phase 4): the WENT layout + bar heights (and,
; for the kernel's benefit, MAXWIN + event types) are generated by unogen from
; unodef/unodef.toml ([world.macplus]) — the SAME include kernel.asm uses, so
; the two sides can no longer drift. Byte-identical to the block this replaced.
