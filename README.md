...
| [**Sony PS2**](ps2/) | PlayStation 2 (MIPS R5900 EE, 32MB, Graphics Synthesizer), FreeMcBoot ELF | Port the portable C core ([mac/unodos.c](mac/unodos.c)) over a software 640×448×32 framebuffer blitted to GS each vsync; DualShock 2 + soft keyboard | **Milestone 1** — the full desktop / WM / all 11 apps (+ FAT12 round-trip, 32-bit Theme) run via the Mac-compat shim over the software FB; verified on the host shim and on the **emulated PS2 GS** in PCSX2 (`ps2/shots/m1_pcsx2_pacman.png`). Remaining: EE audio (audsrv), memory-card storage, real hardware |
