...
with open(OUT, "w", newline="\n") as f:
    ...
    # ---- Theme presets: 8 x (desktop, accent, accent2, text) in BGR555 ----
    THEMES = [
        ((0x00, 0x00, 0xA8), (0x00, 0xA8, 0xA8), (0xA8, 0x00, 0xA8), (0xFF, 0xFF, 0xFF)),  # Classic VGA (= boot)
        ((0x08, 0x08, 0x18), (0xE0, 0x40, 0x40), (0xA0, 0xA0, 0xB0), (0xF0, 0xF0, 0xFF)),  # Midnight
        ((0x06, 0x28, 0x10), (0x30, 0xB0, 0x40), (0x80, 0xE0, 0x60), (0xF0, 0xFF, 0xE0)),  # Forest
        ((0x40, 0x10, 0x10), (0xF0, 0x80, 0x20), (0xE0, 0x40, 0x60), (0xFF, 0xF0, 0xE0)),  # Sunset
        ((0x04, 0x18, 0x40), (0x10, 0xA0, 0xD0), (0x30, 0xE0, 0xC0), (0xE0, 0xF8, 0xFF)),  # Ocean
        ((0x28, 0x28, 0x30), (0x90, 0x90, 0xA0), (0xC8, 0xC8, 0xD0), (0xFF, 0xFF, 0xFF)),  # Slate
        ((0x30, 0x08, 0x28), (0xF0, 0x40, 0xC0), (0xF0, 0xC0, 0x40), (0xFF, 0xF0, 0xF8)),  # Candy
        ((0x18, 0x10, 0x00), (0xF0, 0xA0, 0x10), (0xC0, 0x70, 0x10), (0xFF, 0xF0, 0xD0)),  # Amber
    ]
    thm = []
    for preset in THEMES:
        for (r, g, b) in preset:
            thm.append(bgr555(r, g, b))
    f.write("\n; 8 theme presets x 4 roles (desktop, accent, accent2, text), BGR555\n")
    f.write("thm_presets:\n" + words(thm) + "\n")
